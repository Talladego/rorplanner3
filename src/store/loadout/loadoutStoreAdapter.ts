...
class LoadoutStoreAdapter implements ILoadoutStore {
  getLoadouts(): Loadout[] { return useLoadoutStore.getState().loadouts; }
  getCurrentLoadoutId(): string | null { return useLoadoutStore.getState().currentLoadoutId; }
  getCurrentLoadout(): Loadout | null { return useLoadoutStore.getState().getCurrentLoadout(); }
  getStatsSummary(): StatsSummary { return useLoadoutStore.getState().statsSummary; }
  getActiveSide(): LoadoutSide { return useLoadoutStore.getState().getActiveSide(); }
  getSideLoadoutId(side: LoadoutSide): string | null { return useLoadoutStore.getState().getSideLoadoutId(side); }
  getLoadoutForSide(side: LoadoutSide): Loadout | null { return useLoadoutStore.getState().getLoadoutForSide(side); }
  getSideCareerLoadoutId(side: LoadoutSide, career: Career): string | null { return useLoadoutStore.getState().getSideCareerLoadoutId(side, career); }
  setSideCareerLoadoutId(side: LoadoutSide, career: Career, loadoutId: string | null): void { useLoadoutStore.getState().setSideCareerLoadoutId(side, career, loadoutId); }
  setCareer(career: Career | null): void { useLoadoutStore.getState().setCareer(career); }
  setLevel(level: number): void { useLoadoutStore.getState().setLevel(level); }
  setRenownRank(renownRank: number): void { useLoadoutStore.getState().setRenownRank(renownRank); }
  setItem(slot: EquipSlot, item: Item | null): void { useLoadoutStore.getState().setItem(slot, item); }
  ...
  deleteLoadout(id: string): void { useLoadoutStore.getState().deleteLoadout(id); }
  async switchLoadout(id: string): Promise<void> { await useLoadoutStore.getState().switchLoadout(id); }
  markLoadoutAsModified(id: string): void { useLoadoutStore.getState().markLoadoutAsModified(id); }
  updateLoadoutCharacterStatus(id: string, isFromCharacter: boolean, characterName?: string): void { useLoadoutStore.getState().updateLoadoutCharacterStatus(id, isFromCharacter, characterName); }
}
